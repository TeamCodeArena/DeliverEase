...
def home(request):
    ...
    if request.method == 'POST':
        ...
        return HttpResponseRedirect (reverse('job_details')) # redirects the user to a different url
...
## The code executed when the seller visits it with a GET request
    ...
    jobs = Job.objects.filter(assigned_to=current_user, status='In Progress') # get all the jobs assigned to him
    for job in jobs:
        print(f'posted by: {job.created_by.name}') # print the names of the buyers who created those jobs
